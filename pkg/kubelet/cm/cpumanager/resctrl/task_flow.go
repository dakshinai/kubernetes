...
import (
	...
	"k8s.io/kubernetes/pkg/kubelet/cm/cpumanager/resctrl/task"
)
...
func (t TasksTask) Rollback() error {
	...
	if ok {
		...
		ts = SubtractStringSlice(res.Tasks, t.ResAssociation.Tasks)
		if len(ts) > 0 {
			// Write back to origin
			for _, v := range ts {
				err := writeFile(t.Path, "tasks", v)
				if err != nil {
					glog.Warningf("Failed to add back task %s wile try to rollbak task for %s", v, t.Group)
				}
			}
		} else {
			// Find if new commit task has more tasks than origin one.
			// Remove them to default group
			ts = SubtractStringSlice(t.ResAssociation.Tasks, res.Tasks)
...
func taskFlow(group string, r *ResAssociation, rs map[string]*ResAssociation) error {
	...
	if err := taskList.Start(); err != nil {
		glog.Errorf("Failed to execute task list %s", err.Error())
		return err
	}
	return nil
}
